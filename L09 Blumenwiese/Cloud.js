    ...
    class Cloud {
        constructor(_size, _position) {
            if (_position)
                ...
                this.position = new L09_BlumenwieseUpdate.Vector(50, 50);
            this.velocity = new L09_BlumenwieseUpdate.Vector(30, 0);
            this.size = _size;
        }
        move(_timeslice) {
        ...
        draw() {
            ...
            let gradient = L09_BlumenwieseUpdate.crc2.createRadialGradient(0, 0, 0, 0, 0, radiusParticle);
            particle.arc(0, 0, radiusParticle, 0, 2 * Math.PI);
            gradient.addColorStop(0, "HSLA(0, 100%, 100%, 0.5)");
            gradient.addColorStop(1, "HSLA(0, 100%, 100%, 0)");
            L09_BlumenwieseUpdate.crc2.save();
            L09_BlumenwieseUpdate.crc2.translate(this.position.x, this.position.y);
            L09_BlumenwieseUpdate.crc2.fillStyle = gradient;
            for (let drawn = 0; drawn < nParticles; drawn++) {
                L09_BlumenwieseUpdate.crc2.save();
                L09_BlumenwieseUpdate.crc2.translate((Math.random() - 0.5) * this.size.x, -(Math.random() * this.size.y));
                L09_BlumenwieseUpdate.crc2.fill(particle);
                L09_BlumenwieseUpdate.crc2.restore();
            }
            L09_BlumenwieseUpdate.crc2.restore();
        }
    }
    L09_BlumenwieseUpdate.Cloud = Cloud;
